      ...
      <div class="col">
        <div class="card">
          <div class="card-header">
            <strong> Salary Date List </strong>
          </div>
          <div class="card-body">
            <b-button v-b-modal.createModal variant="info" ref="btnShow"
              >Create Salary Date</b-button
            >
            <input
              type="text"
...
import CreateComponent from "./CreateSalarydates.vue";
import EditComponent from "./EditSalarydates.vue";
...
export default {
  ...
  computed: {
    filteredAbsence() {
      if (!this.filterText == '') {
        return this.apiData.filter((data) => {
          return data.salary_date.toLowerCase().includes(this.filterText.toLowerCase())
        });
      } else {
        return this.apiData;
      }
    ...
    methods: {
      ...
      runAxiosGet() {
        axios
          .get("api/references/salarydates")
          .then((res) => {
      ...
      runAxiosDelete(id) {
        axios
          .delete("api/references/salarydates/" + id)
          .then((res) => {
            this.runAxiosGet();
          })
          .catch((error) => {
