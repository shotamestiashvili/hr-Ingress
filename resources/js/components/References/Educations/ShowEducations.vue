...
import CreateComponent from "./CreateEducations.vue";
import EditComponent from "./EditEducations.vue";
...
export default {
  ...
  computed: {
    filteredAbsence() {
      if (!this.filterText == '') {
        return this.apiData.filter((data) => {
          return data.education.toLowerCase().includes(this.filterText.toLowerCase())
    ...
    methods: {
      ...
      runAxiosDelete(id) {
        axios
          .delete("api/references/educations/" + id)
          .then((res) => {
            this.runAxiosGet();
          })
          .catch((error) => {
