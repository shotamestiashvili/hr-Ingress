...
import CreateComponent from "./CreateQualifications.vue";
import EditComponent from "./EditQualifications.vue";
...
export default {
  ...
  computed: {
    filteredAbsence() {
      if (!this.filterText == '') {
        return this.apiData.filter((data) => {
          return data.qualification.toLowerCase().includes(this.filterText.toLowerCase())
        });
      } else {
        return this.apiData;
      }
    ...
    methods: {
      ...
      runAxiosGet() {
        axios
          .get("api/references/qualifications")
          .then((res) => {
      ...
      runAxiosDelete(id) {
        axios
          .delete("api/references/qualifications/" + id)
          .then((res) => {
            this.runAxiosGet();
          })
          .catch((error) => {
