...
import CreateComponent from "./CreateTrainings.vue";
import EditComponent from "./EditTrainings.vue";
...
export default {
  ...
  computed: {
    filteredAbsence() {
      if (!this.filterText == '') {
        return this.apiData.filter((data) => {
          return data.training.toLowerCase().includes(this.filterText.toLowerCase())
        });
      } else {
        return this.apiData;
      }
    ...
    methods: {
      ...
      runAxiosDelete(id) {
        axios
          .delete("api/references/trainings/" + id)
          .then((res) => {
            this.runAxiosGet();
          })
          .catch((error) => {
